...
api.interceptors.response.use(
  ...
  (err) => {
    ...
    if (err.response?.status === 401) {
      if (err.config?.url !== '/auth/me' && window.location.pathname !== '/login' && window.location.pathname !== '/register') {
...
export const studentApi = {
  ...
  createGroup: (assignmentId, data) => api.post(`/groups/assignment/${assignmentId}`, data),
  addGroupMember: (groupId, data) => api.post(`/groups/${groupId}/members`, data),
  removeGroupMember: (groupId, userId) => api.delete(`/groups/${groupId}/members/${userId}`),
  trackSubmissionClick: (submissionId) => api.post(`/submissions/${submissionId}/track-click`),
  initiateSubmission: (submissionId) => api.post(`/submissions/${submissionId}/initiate`),
  confirmSubmission: (submissionId, data) => api.post(`/submissions/${submissionId}/confirm`, data),
};
