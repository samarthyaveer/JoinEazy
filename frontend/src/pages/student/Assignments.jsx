  ...
  return (
    <PageShell title="Assignments" subtitle="All assignments assigned to you">
      {assignments.length === 0 ? (
        <EmptyState icon={FileText} title="No assignments yet" description="Your professor hasn't posted any assignments yet." />
      ) : (
          ...
          <table className="w-full min-w-[600px]">
            <thead>
              <tr className="border-b border-border bg-surface-overlay/60">
                ...
                <th className="text-left px-4 py-3 text-label text-text-tertiary uppercase tracking-widest hidden sm:table-cell">Due Date</th>
                <th className="text-left px-4 py-3 text-label text-text-tertiary uppercase tracking-widest">Status</th>
              ...
              {assignments.map((a) => (
                <tr key={a.id} className="hover:bg-surface-overlay/50 transition-colors">
                  <td className="px-4 py-4">
                    ...
                    {a.description ? (
                      <p className="text-meta text-text-secondary mt-0.5 line-clamp-1">{a.description}</p>
                    ) : null}
                     {/* Show due date on mobile only */}
                     <div className="sm:hidden mt-1 flex items-center">
                        <span className={`text-meta font-mono tabular-nums ${isPastDue(a.due_date) ? 'text-semantic-danger font-medium' : 'text-text-secondary'}`}>
                          {dateFmt.format(new Date(a.due_date))}
                        </span>
                        {isPastDue(a.due_date) ? <span className="text-label text-semantic-danger ml-1.5">(overdue)</span> : null}
                     </div>
                  </td>
                  ...
                  <td className="px-4 py-4 text-right">
                    <Link to={`/assignments/${a.id}`} className="inline-flex items-center gap-1 text-meta text-text-secondary hover:text-accent transition-colors">
                      View <ChevronRight size={14} className="hidden sm:block" aria-hidden="true" />
                    </Link>
                  </td>
                </tr>
              ))}
