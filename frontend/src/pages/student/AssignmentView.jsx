    ...
    try {
      ...
      setError(
        err?.message ||
          ...
          "Failed to create group. Try a different name.",
      );
    } finally {
      setCreating(false);
    }
    ...
    try {
      ...
      setMemberMsg({
        ...
        text:
          err?.message ||
          err.response?.data?.error ||
          "Failed to add member. Check the email address.",
      });
    } finally {
  ...
  const handleConfirm = async (e) => {
    ...
    try {
      ...
      setSubMsg(
        err?.message ||
          err.response?.data?.error ||
          "Confirmation failed. Check the title and try again.",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    >
              ...
              {submission.status === "submitted" ? (
                <div className="space-y-6">
                  <div className="flex items-center gap-3 p-4 bg-semantic-success/10 rounded-xl border border-semantic-success/20">
                    <CheckCircle2
                      size={20}
                      className="text-semantic-success flex-shrink-0"
                      aria-hidden="true"
                    />
                    <div>
                      ...
                      <p className="text-meta text-text-secondary mt-0.5">
                        {submission.confirmed_at
                          ? `Confirmed on ${dateTimeFmt.format(new Date(submission.confirmed_at))}`
                          : null}
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  {/* Step 1 */}
                  <div
                    className={`flex items-start gap-4 p-4 rounded-xl border ${isStepDone(1) ? "bg-accent/8 border-accent/20" : "bg-surface-overlay border-border"}`}
                  >
                    <div
                      className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${isStepDone(1) ? "bg-accent text-white" : "bg-surface-overlay text-text-tertiary border border-border-strong"}`}
                    >
                      {isStepDone(1) ? (
                        <CheckCircle2 size={14} aria-hidden="true" />
                      ) : (
                        <span className="text-label font-semibold">1</span>
                      )}
                    </div>
                    <div className="flex-1">
                      <p className="text-body font-medium text-text-primary">
                        Open Submission Link & Upload
                      </p>
                      <p className="text-meta text-text-secondary mt-0.5">
                        Click to open OneDrive and upload your files
                      </p>
                      <button
                  ...
                  {/* Step 2 */}
                  <div
                    className={`flex items-start gap-4 p-4 rounded-xl border ${isStepDone(2) ? "bg-accent/8 border-accent/20" : "bg-surface-overlay border-border"}`}
                  >
                    <div
                      className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${isStepDone(2) ? "bg-accent text-white" : "bg-surface-overlay text-text-tertiary border border-border-strong"}`}
                    >
                      {isStepDone(2) ? (
                        <CheckCircle2 size={14} aria-hidden="true" />
                      ) : (
                        <span className="text-label font-semibold">2</span>
                      )}
                    </div>
                    <div className="flex-1">
                      <p className="text-body font-medium text-text-primary">
                      ...
                      <p className="text-meta text-text-secondary mt-0.5">
                        Click once your files are uploaded
                      </p>
                      <button
                  ...
                  {subMsg ? (
                    <p className="text-meta text-semantic-danger px-1">
                      {subMsg}
                    </p>
                  ) : null}
                </div>
        ...
        <div className="space-y-4">
          {myGroup ? (
            <div className="card p-5">
              ...
              <p className="text-meta text-text-secondary mt-0.5 mb-4 capitalize">
                Role: {myGroup.my_role}
              </p>
              ...
              {myGroup.my_role === "leader" ? (
                ...
                >
                  <label
                    className="block text-label font-medium text-text-tertiary mb-2 uppercase tracking-widest"
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      value={addEmail}
                      onChange={(e) =>
                        setAddEmail(e.target.value.toLowerCase())
                      }
                      required
                      style={{ height: "36px" }}
                    />
                  ...
                  {memberMsg.text ? (
                    <p
                      className={`text-meta mt-2 ${memberMsg.type === "error" ? "text-semantic-danger" : "text-semantic-success"}`}
                    >
                      {memberMsg.text}
                    </p>
                  ) : null}
          ...
          <div className="card p-5">
            ...
            <dl className="space-y-3">
              <div>
                <dt className="text-label text-text-tertiary uppercase tracking-widest">
                  Posted By
                </dt>
                <dd className="text-body font-medium text-text-primary mt-0.5">
              ...
              <div>
                ...
                <dd
                  className={`text-body mt-0.5 ${isPastDue ? "text-semantic-danger font-medium" : "text-text-primary"}`}
                >
              ...
              <div>
                ...
                <dd
                  className="text-body text-text-primary mt-0.5 font-mono"
      ...
      >
        <form onSubmit={handleConfirm} className="space-y-5">
          ...
          />
          {subMsg ? (
            <p className="text-meta text-semantic-danger">{subMsg}</p>
          ) : null}
          <div className="flex justify-end gap-3">
            <button
              type="button"
